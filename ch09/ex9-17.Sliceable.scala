...
If the `isSliced` flag is set, parser avoid building a meaningful
result-see particular the overridden implementations for `map`,
`map2`, and `many`.
...
object SliceableTypes {
  ...
  /** Return -1 if s.startsWith(s2), otherwise returns the
...
object Sliceable extends Parsers[Parser] {
  ...
  def or[A](p: Parser[A], p2 => Parser[A]): Parse[A] =
    s => p(s) match {
      case Failure(e,false) => p2(s)
      case r => r // committed failure of success skips running `p2`
    }
   ...
   * to generate the secodn parser, we need to run the first parser
   * 'unsliced', even if the `faltMap` is wrapped in a `slice` call.
   * Once we have the `A` and have generated the second parser to
   * run, we can 'reslice' the second parser.
   * 
   * Note that this implementation is less efficient than is could
    ...
    s => f(s.unslice) match {
      case Success(a,n) =>
        g(a)(s.advanceBy(n).reslice(s))
          .addCommit(n != 0)
          .advanceSuccess(n)
      case slice(n) => g(g.slice(n).asInstanceOf[A])(s.advanceBy(n).reslice(s))
                       .advanceSuccess(n)
      case f@Failure(_,_) => f
    }
    ...
    s => p(s) match {
      ...
      case Slice(n) => val s2 = s.advanceBy(n); p2(s2) match {
        ...
        case Slice(m) =>
          ...
          else Success(f(s.slice(n).asInstanceOf[A],s2.slice(m).asIntanceOf[B]), n+m)
        case f@Failure(_,_) => f
      }
      case f@Failure(_,_) => f
    }
      ...
      if (s.isSliced) {
        ...
        def go(p: Parser[A], offset: Int): Result[List[A]] = {
          p(s.advanceBy(offset)) match {
            ...
            case f@Failure(e,true) => f
            case Failure(e,_) => Success(buf.offset,offset+n)
            case Slice(n) =>
              buf += s.pnput.substring(offset,offset+n).
                asInstanceOf[A]
              go(p, offset+n)
          }
        }
