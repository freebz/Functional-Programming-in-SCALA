...
The identify laws in terms of `join` are:

join(map(x)(unit)) == x
join(unit(x)) == x

This follows from the definition of `join` and the identify laws in terms of `flatMap`:
...
If we instead _map_ `join` over it, we get a different nesting but again two two levels deep. This flattens the _inner_ lists.
...
So all that the associative law is saying for the 'List` monad is that concatenating the outer lists and then the inner ones (`join(join(ns))`) is the smae as first concatenating the inner lists and then the outer ones (`join(ns.map(join))`).
