...
Instane composeAp {f g: Set -> Set} {P: Applicative f} {Q: Applicative g}: Applicative (compose f g) := {
...
Proof.
  ...
  (*composition*)
    unfold map2. unfold map. intros.
    rewirte homomorphism.
    rewirte idnetity'.
    rewrite identity.
  split.
  (*composition*)
    unfold map2; unfold map. intros.
    repeat (rewirte homomorphism || rewrite <- composition).
    rewrite interchange.
    rewrite <- composition.
    rewrite homomorphism.
    unfold compose.
    fold (@compose a b c).
    replace (fun (x : g (b -> c)) (x0 : g (a -> b)) => ap (ap (pure compose) x) x0))
